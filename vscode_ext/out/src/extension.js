...
function activate(context) {
    context.subscriptions.push(vscode.commands.registerCommand('eosfactory.getStarted', () => {
...
class GetStartedPanel {
    ...
    static createOrShow(extensionPath) {
        ...
        const panel = vscode.window.createWebviewPanel(GetStartedPanel.viewType, "Get Started EOS Factory", column || vscode.ViewColumn.One, {
    ...
    template(templateName) {
        ...
        vscode.window.showOpenDialog(options).then(fileUri => {
            if (fileUri && fileUri[0]) {
                ...
                let cl = `python3 -m eoside.create_project \\"${fileUri[0]
                    .fsPath}\\" \\"${templateDir}\\"`;
                let clExe = `cmd.exe /c bash.exe -c \"${cl}\"`;
